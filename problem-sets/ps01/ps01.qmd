...
You may either type or hand write your answers.
...
The `mtcars` dataset in `R` includes the variables `mpg` which measures a car make's fuel efficiency (in miles per gallon) and `wt` which measures the weight (in 100s of lbs).
The following is the output of the regression with the equation: 
$$ mpg_i = \beta_0 + \beta_1 wt_i + u_i $$
