...
geom_histogram(data = line_df, aes(intercept), fill = hp, alpha = 0.75, bins = 40) +
  ...
  scale_x_continuous(breaks = lm0$coefficients[1], labels = latex2exp::TeX(r'($\beta_0$)')) +
...
geom_histogram(data = line_df, aes(slope), fill = hp, alpha = 0.75, bins = 40) +
  geom_vline(xintercept = lm0$coefficients[2], size = 2, color = hi) +
  scale_x_continuous(breaks = lm0$coefficients[2], labels = latex2exp::TeX(r'($\beta_1$)')) +
  theme(axis.text.x = element_text(size = 50),
      axis.text.y = element_blank(),
      rect = element_blank(),
      axis.title.y = element_blank(),
