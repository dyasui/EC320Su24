...
How do I interpret $\hat{\beta_0}$ [=]{.mono} `r round(lm0$coefficients[1], 2)` and $\hat{\beta_1}$ [=]{.mono} `r round(lm0$coefficients[2], 2)`
...
What does $\hat{\beta_0}$ [=]{.mono} `r round(lm0$coefficients[1], 2)` tell us? [Without any police on campus, the crime rate is `r round(lm0$coefficients[1], 2)` per 1,000 people on campus]{.note .fragment}

. . .

What does $\hat{\beta_1}$ [=]{.mono} `r round(lm0$coefficients[2], 2)` tell us? [For each additional police officer per 1,000, there is an associated increase in the crime rate by `r round(lm0$coefficients[2], 2)` crimes per 1,000 people on campus.]{.note .fragment}
