...
::: {.column width="70%"}
Regression Model: $Y_i = \beta_0 + \beta_2 X_i + u_i$<br>
Fitted Line: $\hat{Y_i}= b_0 + b_1 X_i$

Lets calculate the estimated parameters $b_0$ and $b_1$ using the OLS estimator
:::
...
1. Calculate $\hat{X}$ and $\hat{Y}$
2. Compute the intercept estimate $\hat{\beta}_0$ with the intercept formula:

$$
\hat{\beta}_0 = \bar{Y} - \hat{\beta}_1 \bar{X}
$$

3. Compute the slope estimate $\hat{\beta_2}$ using the slope formula:
...
$$
\begin{aligned}
\hat{u}_1 &= Y_1 - \hat{Y_1} = Y_1 - b_0 -b_1 X_1 \\
\hat{u}_2 &= Y_2 - \hat{Y_2} = Y_2 - b_0 -b_1 X_2\\
\hat{u}_3 &= Y_3 - \hat{Y_3} = Y_3 - b_0 -b_1 X_3\\
\hat{u}_4 &= Y_4 - \hat{Y_4} = Y_4 - b_0 -b_1 X_4
\end{aligned}
$$
...
$$
\begin{aligned}
\hat{u}_1 &= 4 - b_0 -1*b_1 \\
\hat{u}_2 &= 3 - b_0 -2*b_1\\
\hat{u}_3 &= 5 - b_0 -3*b_1\\
\hat{u}_4 &= 8 - b_0 -4*b_1
\end{aligned}
$$
...
&= (4 - b_0 - b_1)^2 + (3 - b_0 -2b_1)^2 + (5 - b_0 -3b_1)^2 + (8 - b_0 -4b_1)^2\\
& = 114 + 4b_0^2 + 30b_1^2 - 40b_0 - 114b_1 + 20b_0 b_1 
\end{aligned}
$$
...
$$
\begin{align}
\frac{\partial RSS}{\partial b_0} = 0 &\implies  (4*2)b_0^{2-1} -(40*1)b_0^{1-1} + (20*1) b_0^{1-1} b_1 = 0 \notag \\
&\implies 8b_0 - 40 + 20 b_1 = 0  \ \ \ \ \ \ \  \ \ \ \ \ \ Eq(1)
\end{align}
$$
...
$$
\begin{align}
\frac{\partial RSS}{\partial b_1} = 0 &\implies  (30*2)b_1^{2-1} -(114*1)b_1^{1-1} + (20*1) b_0 b_1^{1-1} = 0 \notag\\
&\implies 60b_1 - 114 + 20 b_0 = 0  \ \ \ \ \ \ \  \ \ \ \ \ \ Eq(2)
\end{align}
$$
...
With two unknowns $\{b_0, b_1\}$ and two equations in which these unknowns satisfied the first order conditions $\left\{\frac{\partial RSS}{\partial b_0}, \frac{\partial RSS}{\partial b_1}\right\}$, we can solve for our parameters.

How? Substitute one expression into the other. 

$$
\begin{aligned}
20 b_1 &= 40 - 8b_0  \implies 60b_1 = 120 - 24b_0\\
&\text{substitute into second equation}\\
 Eq(2): \  & 120 - 24b_0 -114 + 20b_0 = 0\\
& 6 = 4b_0 \implies b_0 = 1.5\\
Eq(1): \  & 20b_1 = 40 - 8\times 1.5 = 28 \implies b_1 = 1.4
\end{aligned}
$$
