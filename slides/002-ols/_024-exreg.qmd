...
data %>% 
  ggplot(aes(x = police, y = crime)) +
    geom_point(color = hi, size = 1.5, alpha = 0.8) +
    mytheme_s +
...
$$
{\color{#81A1C1} \text{Crime}_i} = \beta_0 + \beta_1 {\color{#B48EAD} \text{Police}_i} + u_i.
$$
...
$$
\text{Crime}_i = {\color{#BF616A} \beta_0} + \beta_1\text{Police}_i + u_i
$$
...
[Let's take some guesses: ${\color{#ffffff} \hat{\beta_0} = 60}$]{.fragment}
...
Let's take some guesses: $\hat{\beta_0} = 60$ and $\hat{\beta_1}=-7$
...
Let's take some guesses: $\hat{\beta_0} = 30$ and $\hat{\beta_1}=0$
...
Let's take some guesses: $\hat{\beta_0} = 15.6$ and $\hat{\beta_1}=7.94$
...
Using $\hat{\beta_0}$ and $\hat{\beta_1}$ to make $\hat{Y_i}$ generates misses.
...
$\hat{\beta_0} = 15.4$ and $\hat{\beta_1}=7.94$
...
We could test thousands of guesses of $\beta_0$ and $\beta_1$ and pick the pair the has the smallest RSS

. . .

What if we did that. [Let's write a loop that guesses $\beta_0$ and $\beta_1$ [100,000 times]{.hi} and collect the [RSS]{.hii} for each guess.]{.fragment}

[Then we plot it in three dimensions: $\beta_0$ ([x]{.mono}), $\beta_1$ ([y]{.mono}), [RSS]{.hii} ([z]{.mono})]{.fragment}
...
Simulated RSS across $\beta_0, \beta_1 \sim \text{uniform}(-100, 100)$
...
Simulated RSS across $\beta_0, \beta_1 \sim \text{uniform}(-100, 100)$ _(zoomed in)_
...
What if we did that. Let's write a loop that guesses $\beta_0$ and $\beta_1$ [100,000 times]{.hi} and collect the [RSS]{.hii} for each guess.

Then we plot it in three dimensions: $\beta_0$ ([x]{.mono}), $\beta_1$ ([y]{.mono}), [RSS]{.hii} ([z]{.mono})
