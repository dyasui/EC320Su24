...
geom_text(aes(x = xl, y = yl, label = l), size = 12, parse = T) +
xlim(-2.5, 2) +
ylim(-4.5, 2) +
coord_equal()
```
...
::: {.column width="70%"}
$$
{\color{#ffffff} \overline{\text{mpg}_i} = 20.1}
$$
:::
...
::: {.column width="40%"}
$$
{\color{#B48EAD} \widehat{\text{mpg}_i}} = 37.3 - 5.3 \cdot \text{wt}_i 
$$
:::
...
::: {.column width="60%"}
$$
{\color{#ffffff} \widehat{\text{mpg}_i}} = 37.3 - 5.3 \cdot \text{wt}_i 
$$
:::
...
mtcars %>% 
  ggplot(aes(x = wt, y = mpg)) +
    geom_hline(yintercept = y_bar, color = higreen, size = 1) +
    geom_abline(intercept = b0, slope = b1, color = hp, size = 1.25) +
    geom_segment(aes(x = wt+0.02, xend = wt+0.02, y = y_bar, yend = y_hat(wt, b0, b1)), color = hiyellow, position = position_dodge(-1000), alpha = 1, size = 2) +
    geom_segment(aes(x = wt, xend = wt, y = mpg, yend = y_bar), color = hired, alpha = 1, size = 2) +
    geom_segment(aes(x = wt-0.02, xend = wt-0.02, y = mpg, yend = y_hat(wt, b0, b1)), color = hiorange, position = position_dodge(1000), alpha = 1, size = 2) +
    geom_point(color = hi) +
    # coord_fixed() +
    coord_cartesian(xlim = c(1,6), ylim = c(0,35)) +
    scale_size(range = c(0.001,2)) +
...
![](./images/khoavu-tweet.png){fig-align="center"}
